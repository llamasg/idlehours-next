...
export default async function GameDetailPage({
  ...
  const { slug } = await params
  const [game, allGames] = await Promise.all([getGame(slug), getAllGames()])

  if (!game) notFound()

  const related: Game[] = (allGames ?? [])
    .filter((g: Game) => g.slug.current !== slug)
    .slice(0, 4)
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 lg:px-8 lg:py-12">
        ...
        <div className="overflow-hidden rounded-2xl border border-border/60 bg-card shadow-sm">
          <div className="grid gap-0 lg:grid-cols-[1.4fr_1fr]">
            {/* Cover image */}
            <div className="relative aspect-[16/9] overflow-hidden bg-secondary lg:aspect-auto lg:min-h-full">
              {game.coverImage ? (
